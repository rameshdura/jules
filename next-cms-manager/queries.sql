...
-- 1. Fetching a single complete published post with all its relations nested in JSON
...
WHERE p.slug = $1 AND pt.slug = $2 AND p.status = 'published';
...
-- 3. Inserting a Post and all its relations within a Single CTE Query
-- This is highly useful, executable as-is, and completely safe without needing client-side extraction variables!
WITH inserted_post AS (
    ...
    VALUES (
        (SELECT id FROM post_types WHERE slug = 'blog' LIMIT 1),
    ...
    RETURNING id, post_type_id
),
inserted_category AS (
    INSERT INTO post_categories (post_id, category_id, post_type_id)
    SELECT ip.id, c.id, ip.post_type_id
    FROM inserted_post ip
    CROSS JOIN categories c
    WHERE c.slug = 'technology' AND c.post_type_id = ip.post_type_id
),
inserted_media AS (
    INSERT INTO post_media (post_id, media_type, url, title, alt_text, mime_type, file_size, sort_order)
    SELECT ip.id, 'image', 'https://example.com/uploads/pic1.jpg', 'Nice Pic', 'A beautiful landscape', 'image/jpeg', 102400, 1
    FROM inserted_post ip
),
inserted_file AS (
    INSERT INTO post_files (post_id, url, title, description, mime_type, file_size, sort_order)
    SELECT ip.id, 'https://example.com/uploads/manual.pdf', 'Download PDF Guide', 'User manual in PDF format', 'application/pdf', 2048500, 1
    FROM inserted_post ip
)
INSERT INTO post_links (post_id, url, title, description, target, sort_order)
SELECT ip.id, 'https://nextjs.org', 'Next.js Official Site', 'Visit official page', '_blank', 1
FROM inserted_post ip;


-- 4. Find all posts belonging to a specific category (and its child categories)
-- This recursive CTE fetches the chosen category plus all its subcategories, using UNION to prevent cycles,
-- then joins it to retrieve all associated posts.
WITH RECURSIVE category_tree AS (
    SELECT id, parent_id, post_type_id FROM categories WHERE slug = $1 AND post_type_id = (SELECT id FROM post_types WHERE slug = $2)
    UNION
    SELECT c.id, c.parent_id, c.post_type_id FROM categories c
    INNER JOIN category_tree ct ON c.parent_id = ct.id AND c.post_type_id = ct.post_type_id
)
